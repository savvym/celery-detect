...
class EventBroadcaster(QueueSubscriber[dict]):
    ...
    def parse_event(self, event: dict) -> EventMessage:
        ...
        if event_category == "task":
            return self.parse_task_event(event, event_type)
        elif event_category == "worker":
            return self.parse_worker_event(event, event_type)
        else:
            raise InvalidEventError(f"Unknown event category {event_category!r}")

    def parse_worker_event(self, event: dict, event_type: str) -> EventMessage | None:
    ...
    def parse_task_event(self, event: dict, event_type: str) -> EventMessage | None:
