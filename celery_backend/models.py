...
class ChoreResult(models.Model):
    """Task result/status."""
    rtx_name = models.CharField(db_column='rtx_name',
                                null=False,
                                max_length=255,
                                default='',
                                db_comment="operator's rtx name")
    region = models.CharField(db_column='region',
                              null=False,
                              max_length=64,
                              default='',
                              db_comment='region')
    request_id = models.CharField(db_column='request_id',
                                  null=False,
                                  max_length=64,
                                  default='',
                                  db_comment='Request Id',
                                  verbose_name=_('Request Id'),
                                  help_text=_('Request Id create this task'))
    parent_id = models.CharField(db_column='parent_id',
                                 null=False,
                                 max_length=64,
                                 default='',
                                 db_comment='Parent Chore Id')
    root_id = models.CharField(db_column='root_id',
                               null=False,
                               max_length=64,
                               default='',
                               db_comment='Root Chore Id')

    chore_id = models.CharField(
        max_length=getattr(
            settings,
        ...
        unique=True,
        verbose_name=_('Chore ID'),
        help_text=_('Celery ID for the Chore that was run'))
    ...
    task_name = models.CharField(
        ...
        help_text=_('Name of the Chore which was run'))
    ...
    objects = managers.ChoreResultManager()

    class Meta:
        app_label = 'chore'
        """Table information."""
        db_table = 'ChoreResult'
        ordering = ['-date_done']
    ...
    def as_dict(self):
        return {
            ...
            'rtx_name': self.rtx_name
        }

    def __str__(self):
        return '<Task: {0.chore_id} ({0.status})>'.format(self)
